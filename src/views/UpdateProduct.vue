<template>
{{ getProducts('').filter(el => el.id === $route.params.id)[0] }}
    ...
    <div class="w-full h-5/6 flex items-center justify-center">
      ...
      >
        <div class="mt-5 w-full">
          <h3 class="font-poppins text-3xl font-bold m-5">Update Product</h3>
          <div
        ...
        {{ getItem }}
          ...
          <div class="w-full h-12 mt-14">
            <Button :text="'Update'" :primary="true" />
          </div>
...
export default {
  ...
  computed: {
    ...
    getItem() {
        this.item = this.getProducts('').filter(el => el.id == this.$route.params.id)[0]
    },
  },
  methods: {
